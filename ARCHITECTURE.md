...
**名前解決のコスト**: `collectFolderForDownload` は `listEntries` 経由で各エント
リの名前を復号する。ソルトはエントリごとなので鍵は共有できず（§4）、`nameCache`
に載っていない1件ごとに 60万反復の PBKDF2 が1回走る。UI で展開済みのエントリは
無料だが、「download all」は未展開のサブツリーも歩くのでそこは全件が初回導出に
なる。数百ファイル規模では、この時間が実際の I/O を上回る（実測で `preparing`
が支配的だった）。

`listEntries` は同一ディレクトリ内の子を `NAME_RESOLUTION_CONCURRENCY` 件ずつの
プールで解決する。`crypto.subtle` はメインスレッド外で動くため、並列化で実コア
ぶんの短縮が見込めるうえ、後続の子の OPFS 読みを先行する子の PBKDF2 に重ねられ
る。上限を設けているのは、幅の広いディレクトリで全子のファイルハンドルを同時に
開かないため。なお兄弟ディレクトリ間の再帰は逐次のまま（プールの入れ子を避け
る）。
...
- **バックアップ時に名前解決を省く**: `.lfl` の再インポートは埋め込みの opaque
  id を使うためパスワード不要（§8）。つまり opaque id 名のままの zip でもバック
  アップとしては完全に機能し、実名は人間が中身を見るときの利便性でしかない。
  名前解決を丸ごと飛ばせば PBKDF2 のコストは消えるが、アーカイブの見た目が変わ
  るのでトグルなどの UI 判断が要る。上の並列化で足りなければ検討する。
