...
に載っていない1件ごとに 60万反復の PBKDF2 が**直列で**1回走る。UI で展開済みの
エントリは無料だが、「download all」は未展開のサブツリーも歩くのでそこは全件が
初回導出になる。数百ファイル規模では、この時間が実際の I/O を上回りうる（§12）。
...
- **名前解決の並列化**: `listEntries` は子エントリを逐次 `await` で回すため、
  PBKDF2 が1件ずつ直列に走る（§9）。同時実行数を絞った `Promise.all` にすれば、
  `crypto.subtle` がメインスレッド外で動くぶんの短縮が見込める。フォーマットも
  セキュリティ特性も変わらず、一括DLだけでなく通常のフォルダ展開も速くなる。
  ただし WebKit の crypto キューが実際にコア数ぶん並列化するかは未検証で、効果
  はまず計測してから判断すること。
