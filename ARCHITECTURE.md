...
**進捗と容量チェック**: `predictLength()` はアーカイブの正確なバイト数を中身な
しで返す（各エントリの `Blob.size` がそのまま zip への寄与バイト数なので、
`VaultFileEntry.size`（復号後の論理サイズ）ではなくこちらを渡す）。これを
`navigator.storage.estimate()` と突き合わせて開始前に不足を弾き、同じ値を進捗の
分母に使う。ただし `estimate()` が返すのはオリジンの quota に対する余裕であっ
て、Safari では実際の空き容量よりずっと大きく出うる。よくあるケースを捕まえる
だけで、全ケースは防げない。

進捗は2フェーズに分かれて見える。**preparing**（`total === 0`）の間は
`collectFolderForDownload` がツリーを歩いており、1バイトも流れない。**zipping
N%** に変わってから実際の I/O が走る。前者が長い場合の犯人は下の名前解決コスト。

### アーカイブ内の命名

`collectFolderForDownload` は `listEntries` を通さず、独自にツリーを歩く。
`listEntries` が全エントリの名前を先に復号してしまうためで、代わりに次の方針を
取る。

| 対象 | アーカイブ内の名前 | PBKDF2 |
|---|---|---|
| ディレクトリ | 実名 | 要 |
| v2-framed ファイル | `<opaqueId>.lfl` | **不要** |
| raw-passthrough ファイル | `<実名>.enc` | 要 |

**v2-framed で実名を解決しない根拠**: 実名は書き出したコンテナの中に暗号化され
たまま同梱されており、`importExportedFile` は**埋め込みの opaque id** から復元
してアーカイブ上のファイル名を一切読まない（§8）。つまり名前は失われず、再イン
ポートすれば元通り表示される。アーカイブ上の名前は純粋に見た目の問題。

**raw-passthrough が例外である理由**: こちらの書き出しはヘッダと name ブロック
を剥がして content だけを出すため、**暗号化された名前が書き出しバイト列に残らな
い**。再インポート時に `NewFile.tsx` がファイル名から実名を復元するので、ここで
省くと名前が恒久的に失われる。どちらの分岐かはコンテナヘッダ1バイトで決まり、導
出は不要。

ディレクトリ名を解決するのは、数が少ないうえアーカイブがフォルダ単位で辿れる状
態を保てるため。列挙は opaque id でソートしてから処理する（実名は解決しないので
それが唯一の安定順序であり、アーカイブ構成と `dedupeFilename` の採番を実行ごとに
再現可能にする）。

### 名前解決のコスト

ソルトはエントリごとなので鍵は共有できず（§4）、`nameCache` に載っていない1件ご
とに 60万反復の PBKDF2 が1回走る。UI で展開済みのエントリは無料だが、「download
all」は未展開のサブツリーも歩くのでそこは全件が初回導出になる。上記の命名方針に
より、一括DLでの導出回数は「全ファイル数＋ディレクトリ数」から「ディレクトリ数＋
`.enc` の数」に落ちている。

`listEntries` は同一ディレクトリ内の子を `NAME_RESOLUTION_CONCURRENCY` 件ずつの
プールで解決する。ただし**iOS Safari では並列化の効果はほぼ無かった**（数百ファ
イルの `preparing` が並列化の有無でどちらも約1分40秒）。`crypto.subtle` を単一
キューで直列化しているとみられ、この経路で PBKDF2 の総時間を削ることはできない
——上記の「そもそも導出しない」方針を取ったのはこのため。プール自体は OPFS 読み
を重ねられること、および他ブラウザでの効果を見込んで残してある。上限を設けている
のは、幅の広いディレクトリで全子のファイルハンドルを同時に開かないため。兄弟ディ
レクトリ間の再帰は逐次のまま（プールの入れ子を避ける）。
