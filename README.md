...
Local File Locker is an encrypted file manager that runs entirely within your browser. Files are encrypted on your device and stored using the browser's Origin Private File System. Encryption is performed using symmetric encryption (AES-256-GCM), with the key derived from your password via PBKDF2, and the process runs entirely through the browser's native Web Crypto API.
